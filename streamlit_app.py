...
st.title("📈 Crypto Scalper Backtest – BTC & ETH (1m, interattivo con filtri)")
...
def backtest(df, tp_pct, sl_pct, max_candles, min_entry_flags):
    df = df.copy()
...
with tab1:
    ...
    btc_results = backtest(btc_df, tp_pct, sl_pct, max_candles, min_entry_flags)
...
with tab2:
    ...
    eth_results = backtest(eth_df, tp_pct, sl_pct, max_candles, min_entry_flags)
