...
st.title("📈 Crypto Scalper Backtest – BTC & ETH")

@st.cache_data
def load_data():
    btc = pd.read_csv("BTC_USDC_1m.csv", parse_dates=["timestamp"])
    eth = pd.read_csv("ETH_USDC_1m.csv", parse_dates=["timestamp"])
    return btc, eth
...
def backtest(df):
    ...
    df["green_full"] = (df["close"] > df["open"]) & ((df["close"] - df["open"]) > (df["high"] - df["low"]) * 0.5)
